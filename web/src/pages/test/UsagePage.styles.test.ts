...
describe('UsagePage responsive layout and accessibility', () => {
  ...
  it('keeps Analysis grids and charts within narrow containers', () => {
    ...
    expect(styleRuleBlock(analysisPanelStyles, '.analysisSummary')).toContain('flex-wrap: wrap;')
    expect(styleRuleBlock(analysisPanelStyles, '.compositionLayout')).toContain('flex-direction: column;')
    const donut = styleRuleBlock(analysisPanelStyles, '.donutCanvasBox')
    expect(donut).toContain('width: 100%;')
    expect(donut).toContain('min-width: 0;')
