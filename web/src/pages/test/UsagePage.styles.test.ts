...
describe('UsagePage responsive layout and accessibility', () => {
  ...
  it('keeps Analysis grids and charts within narrow containers', () => {
    const insights = styleRuleBlock(analysisPanelStyles, '.insightGrid')
    expect(insights).toContain('grid-template-columns: repeat(2, minmax(0, 1fr));')
    expect(insights).toMatch(/@include mobile\s*\{[^}]*grid-template-columns:\s*1fr;/)
    const metrics = styleRuleBlock(analysisPanelStyles, '.costMetricGrid')
    expect(metrics).toContain('grid-template-columns: repeat(4, minmax(0, 1fr));')
    expect(metrics).toMatch(/@include tablet\s*\{[^}]*grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);/)
    expect(metrics).toMatch(/@include mobile\s*\{[^}]*grid-template-columns:\s*1fr;/)
    expect(styleRuleBlock(analysisPanelStyles, '.compositionLayout')).toMatch(/@include mobile\s*\{[^}]*grid-template-columns:\s*1fr;/)
    expect(styleRuleBlock(analysisPanelStyles, '.donutChartFrame')).toMatch(/@include mobile\s*\{[^}]*min-height:\s*0;/)
    const donut = styleRuleBlock(analysisPanelStyles, '.donutCanvasBox')
    expect(donut).toContain('width: min(100%, 340px);')
    expect(donut).toContain('height: auto;')
    expect(donut).toContain('aspect-ratio: 1;')
    expect(donut).toContain('max-width: 100%;')
    expect(donut).toMatch(/@include mobile\s*\{[^}]*width:\s*min\(100%, 260px\);/)
    expect(donut).toMatch(/@include mobile\s*\{[^}]*height:\s*auto;/)
    const metadata = styleRuleBlock(analysisPanelStyles, '.compositionUsageMetaPill')
    expect(metadata).toContain('max-width: 100%;')
  ...
  it('keeps Analysis tooltips and heatmap labels accessible', () => {
    ...
    for (const selector of ['.costStackFloatingTooltip', '.heatmapFloatingTooltip']) {
      expect(styleRuleBlock(analysisPanelStyles, selector)).toContain('position: fixed;')
    }
