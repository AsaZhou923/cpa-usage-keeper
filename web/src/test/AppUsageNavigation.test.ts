...
describe('App usage-page route authorization', () => {
  ...
  it('keeps API key viewers isolated from administrator routes', () => {
    ...
    expect(getRoleTargetPath('api_key_viewer', '/key-events')).toBe('/key-events');
    expect(shouldNormalizeRolePath('api_key_viewer', '/key-events')).toBe(false);
    expect(getRoleTargetPath('api_key_viewer', '/key-auth-files')).toBe('/key-auth-files');
    expect(shouldNormalizeRolePath('api_key_viewer', '/key-auth-files')).toBe(false);
  ...
  it('routes API key viewers to scoped event and auth-file pages', () => {
    expect(appSource).toContain("import { KeyEventsPage } from './pages/KeyEventsPage';");
    expect(appSource).toContain("import { KeyAuthFilesPage } from './pages/KeyAuthFilesPage';");
    expect(appSource).toContain("keyViewerPath === '/key-events'");
    expect(appSource).toContain("keyViewerPath === '/key-auth-files'");
    expect(appSource).toContain('<KeyEventsPage');
    expect(appSource).toContain('<KeyAuthFilesPage');
  });
