...
vi.mock('../pages/KeyOverviewPage', () => ({
  KeyOverviewPage: ({ onNavigate, page = 'overview' }: ComponentProps<typeof import('../pages/KeyOverviewPage').KeyOverviewPage>) => (
    <button data-navigate onClick={() => onNavigate('/key-ranking')}>{page === 'realtime' ? 'Realtime' : 'Overview'}</button>
  ),
}));
vi.mock('../pages/KeyAnalysisPage', () => ({ KeyAnalysisPage: () => <div>Analysis</div> }));
vi.mock('../pages/KeyEventsPage', () => ({ KeyEventsPage: () => <div>Events</div> }));
vi.mock('../pages/KeyAuthFilesPage', () => ({ KeyAuthFilesPage: () => <div>Auth files</div> }));
...
describe('App session and navigation', () => {
  ...
  it.each([
    { path: '/key-overview', page: 'Overview' },
    { path: '/key-realtime', page: 'Realtime' },
    { path: '/key-analysis', page: 'Analysis' },
    { path: '/key-events', page: 'Events' },
    { path: '/key-auth-files', page: 'Auth files' },
    { path: '/key-ranking', page: 'Ranking' },
  ])('renders the viewer page for $path without changing its route', async ({ path, page }) => {
