...
const syncPanelSource = readFileSync(new URL('../pricing/PriceSyncPanel.tsx', import.meta.url), 'utf8');
...
describe('PriceSettingsCard', () => {
  ...
  it('keeps sync draft pricing controls immutable while sync apply is pending', () => {
    const syncDraftGridStart = syncPanelSource.indexOf('<div className={styles.syncDraftGrid}>');
    const syncDraftGridEnd = syncPanelSource.indexOf('\n                        </div>\n                      </div>\n                    </div>', syncDraftGridStart);
    const syncDraftGrid = syncPanelSource.slice(syncDraftGridStart, syncDraftGridEnd);
  ...
  it('keeps edit and delete modals locked while persistence is pending', () => {
    ...
    const editModalStart = source.search(/<Modal\s*\n\s*open={editModel !== null}/);
    const editModalEnd = source.indexOf('</Modal>', editModalStart);
    const editModal = source.slice(editModalStart, editModalEnd);
    const deleteModalStart = source.search(/<Modal\s*\n\s*open={deleteModel !== null}/);
    const deleteModalEnd = source.indexOf('</Modal>', deleteModalStart);
