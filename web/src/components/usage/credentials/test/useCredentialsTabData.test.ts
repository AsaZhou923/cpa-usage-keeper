...
const credentialsTabDataSource = readFileSync(new URL('../useCredentialsTabData.ts', import.meta.url), 'utf8')
const resetQuotaForAuthIndexSource = credentialsTabDataSource.match(/const resetQuotaForAuthIndex = useCallback[\s\S]*?\n {2}}, \[onNotice, readOnly, refreshQuotaForAuthIndex\]\)/)?.[0] ?? ''
...
describe('useCredentialsTabData quota response contract', () => {
  it('narrows reset callback dependencies to the read-only guard, refresh callback, and notice handler', () => {
    expect(resetQuotaForAuthIndexSource).toContain('}, [onNotice, readOnly, refreshQuotaForAuthIndex])')
  })

  it('routes reset outcomes through the shared helper and top notice', () => {
    expect(resetQuotaForAuthIndexSource).toContain('runQuotaResetForAuthIndex(authIndex, {')
    expect(resetQuotaForAuthIndexSource).toContain("onNotice?.('error', outcome.message)")
    expect(resetQuotaForAuthIndexSource).not.toContain("onAuthRequired?.()")
    expect(resetQuotaForAuthIndexSource).not.toContain('quotaResetError')
  })

  it('guards credential status mutation when the credentials tab is read-only', () => {
    expect(credentialsTabDataSource).toMatch(/const toggleCredentialStatus = useCallback\(async[\s\S]*?if \(readOnly\) \{\s*return\s*\}/)
    expect(credentialsTabDataSource).toMatch(/}, \[onAuthRequired, onNotice, readOnly\]\)/)
  })
})
