...
describe('CredentialHealthPanel health semantics', () => {
  ...
  it('raises the green threshold logarithmically with the request sample size', () => {
    ...
    expect(html).toContain('11:00 - 11:10|usage_stats.credentials_health_status_success|9/1|90.0%')
    expect(html).toContain('11:10 - 11:20|usage_stats.credentials_health_status_warning|90/10|90.0%')
    expect(html).toContain('11:20 - 11:30|usage_stats.credentials_health_status_success|95/5|95.0%')
    expect(html).toContain('11:30 - 11:40|usage_stats.credentials_health_status_warning|1/1|50.0%')
    expect(html).toContain('11:40 - 11:50|usage_stats.credentials_health_status_failure|1/2|33.3%')
  })
  ...
  it('scales every populated bar continuously by success rate above the empty baseline', () => {
    ...
    expect(html).toContain('aria-label="11:00 - 11:10|usage_stats.credentials_health_status_success|99/1|99.0%" style="height:21.9px"')
    expect(html).toContain('aria-label="11:10 - 11:20|usage_stats.credentials_health_status_success|9/1|90.0%" style="height:20.8px"')
    expect(html).toContain('aria-label="11:20 - 11:30|usage_stats.credentials_health_status_warning|1/1|50.0%" style="height:16px"')
    expect(html).toContain('aria-label="11:30 - 11:40|usage_stats.credentials_health_status_failure|1/2|33.3%" style="height:14px"')
    expect(html).toContain('aria-label="11:40 - 11:50|usage_stats.credentials_health_status_failure|0/1|0.0%" style="height:10px"')
  ...
  it('uses exact request totals when the API success rate is unavailable', () => {
    ...
    expect(html).toContain('Failed requests in 5h: 1 · latest 11:00 - 11:10')
