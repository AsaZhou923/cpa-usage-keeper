...
describe('AuthFileCredentialsSection quota reset formatting', () => {
  it.each([
    ['2026-05-12T10:15:00Z', '2d0h15m'],
    ['2026-05-10T14:15:00Z', '4h15m'],
  ])('formats reset %s and remaining duration %s', (resetAt, duration) => {
...
describe('AuthFileCredentialsSection quota usage mode rendering', () => {
  ...
  it('keeps the reset time visible when Codex has no token or cost usage', () => {
    ...
    expect(html).toContain(formatLocalResetTime('2026-05-09T12:00:00Z'))
  })
