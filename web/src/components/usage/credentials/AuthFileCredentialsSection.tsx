...
export function isInspectionStartDisabled({ quotaAutoRefreshEnabled = false, starting, total, running }: { quotaAutoRefreshEnabled?: boolean; starting: boolean; total: number; running: boolean }): boolean {
...
export function QuotaInspectionModal({
  ...
  // startDisabled 只依赖显式巡检状态、自动刷新开关和可巡检总数，不被普通行刷新状态牵连。
  ...
  const inspectionRunning = starting || status?.running
