...
func (s identityMetadataStub) ListActiveUsageIdentityAuthIndexes(context.Context, service.ListUsageIdentitiesRequest) ([]string, error) {
...
func (s identityMetadataStub) UpdateUsageIdentityAlias(context.Context, int64, string) (entities.UsageIdentity, error) {
	if len(s.items) == 0 {
...
func TestUsageIdentitiesRouteReturnsMetadataStatsAndActiveRows(t *testing.T) {
	...
	activeIdentity := entities.UsageIdentity{
		...
		Prefix:                     "claude-team",
		Priority:                   testPtr(4),
		Disabled:                   testPtr(true),
		Note:                       testPtr("desktop note"),
		TotalRequests:              10,
		SuccessCount:               8,
		FailureCount:               2,
		InputTokens:                100,
...
func TestUsageIdentitiesPageRouteFiltersByAuthTypeAndPaginates(t *testing.T) {
	...
	router := NewRouter(nil, nil, nil, nil, AuthConfig{}, nil, "", OptionalProviders{UsageIdentity: identityMetadataStub{
		...
		pagedActiveItems: []entities.UsageIdentity{{
			...
			Type:         "codex",
			Provider:     "Codex",
			FileName:     testPtr("codex-user.json"),
			FilePath:     testPtr("/data/auths/codex-user.json"),
		}},
	}})
...
func TestUsageIdentitiesRoutePublishesAIProviderAuthIndexWithoutLookupKey(t *testing.T) {
	...
	router := NewRouter(nil, nil, nil, nil, AuthConfig{}, nil, "", OptionalProviders{UsageIdentity: identityMetadataStub{items: []entities.UsageIdentity{
		{ID: 1, Name: "Provider Name", Prefix: "Team Prefix", AuthType: entities.UsageIdentityAuthTypeAIProvider, AuthTypeName: "apikey", Identity: authIndex, LookupKey: lookupKey, Type: "openai", Provider: "OpenAI", FileName: testPtr("should-not-return.json"), FilePath: testPtr("/data/auths/should-not-return.json")},
	}}})
	resp := serveAPIGet(router, "/api/v1/usage/identities")
