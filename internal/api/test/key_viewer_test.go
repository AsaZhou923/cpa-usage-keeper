...
func newKeyViewerRouteTestRouter(t *testing.T, usageProvider service.UsageProvider, providers OptionalProviders) (*httptest.ResponseRecorder, *http.Cookie, http.Handler) {
	...
	return httptest.NewRecorder(), &http.Cookie{Name: standardSessionCookieName, Value: token}, NewRouter(nil, nil, usageProvider, nil, config, handler, "", providers)
}

func encodeExpectedUsageEventsCursor(timestamp time.Time, id int64) string {
...
func TestKeyOverviewEventsForceCurrentViewerAPIKey(t *testing.T) {
	...
	expectedCursor := encodeExpectedUsageEventsCursor(eventTime, 7)
...
func TestKeyOverviewAuthFilesPageScopesToCurrentAPIGroupKey(t *testing.T) {
	...
	identityProvider := identityMetadataStub{
...
func TestKeyOverviewQuotaCacheRejectsUnauthorizedAuthIndex(t *testing.T) {
	...
	identityProvider := identityMetadataStub{items: []entities.UsageIdentity{{Identity: "auth-1"}}}
